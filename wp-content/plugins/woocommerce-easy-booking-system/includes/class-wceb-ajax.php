...
if ( ! class_exists( 'EasyBooking\Ajax' ) ) :
...
class Ajax {
	...
	public function __construct() {
        ...
        add_action( 'wp_ajax_clear_booking_session', array( $this, 'clear_booking_session' ) );
        add_action( 'wp_ajax_nopriv_clear_booking_session', array( $this, 'clear_booking_session' ) );
        add_action( 'wceb_ajax_clear_booking_session', array( $this, 'clear_booking_session' ) );
    ...
    **/
    public static function set_booking_session() {
        ...
        **/
        ...
        if ( $number_of_dates === 'one' && ! empty( $end ) ) { // One date: check that end date is not set.

            self::get_error_fragments( __( 'You can only select one date.', 'woocommerce-easy-booking-system' ) );

        } else if ( $number_of_dates === 'one' && empty( $start ) ) { // One date: check that start date is set.

            self::get_error_fragments( __( 'Please select a date.', 'woocommerce-easy-booking-system' ) );

        } else if ( $number_of_dates === 'two' && ( empty( $start ) || empty( $end ) ) ) { // Two dates: check that both dates are set.

            self::get_error_fragments( __( 'Please select dates.', 'woocommerce-easy-booking-system' ) );
        ...
        **/
        ...
        $duration = $number_of_dates === 'one' ? 1 : Date_Selection::get_selected_booking_duration( $start, $end, $_product );
    ...
    *
    **/
    public static function clear_booking_session() {
    ...
    private static function get_success_fragments( $id, $booking_data ) {
        ...
        */
        foreach ( $booking_data as $_product_id => $_product_booking_data ) {
            ...
            $qty = $product->is_type( 'bundle' ) ? 1 : $_product_booking_data['quantity'];
        ...
        if ( $product->is_type( 'bundle' ) ) {
            ...
            $bundle_qty = $booking_data[$id]['quantity'];

            $new_price *= $bundle_qty;
            $new_regular_price *= $bundle_qty;
