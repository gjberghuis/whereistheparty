    ...
    **/
    public function wceb_new_price_fragment( $id, $children, $booking_data ) {
        ...
        if ( isset( $new_regular_price ) && ( $new_regular_price != $new_price ) ) {
            $new_regular_price = $tax_display_mode === 'incl' ? $product->get_price_including_tax( 1, $new_regular_price ) : $product->get_price_excluding_tax( 1, $new_regular_price ); // Product price (Regular or sale)
        } else {
            $new_regular_price = '';
        }
        ...
        if ( wceb_get_product_booking_dates( $product ) === 'two' ) {
            ...
            if ( $booking_duration === 'weeks' ) {
                $unit = _n( 'week', 'weken', $duration, 'easy_booking' );
            } else {
                $unit = $calc_mode === 'nights' ? _n( 'nacht', 'nachten', $duration, 'easy_booking' ) : _n( 'dag', 'dagen', $duration, 'easy_booking' );
            }

            $details .= '<p class="booking_details">';
            $details .= sprintf(
                __( 'Totate boekingsduur: %s %s', 'easy_booking' ),
                absint( $duration ),
                esc_html( $unit )
            );
