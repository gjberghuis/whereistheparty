...
(function($) {
	...
	$(document).ready( function() {
		...
		*/
		wceb.clearBookingSession = function() {
			...
			if ( wceb.session ) {
				...
				$.post( ajax_url.toString().replace( '%%endpoint%%', 'clear_booking_session' ), data, function() {
					wceb.session = false;
				});
		...
		* WooCommerce Product Add-ons compatibility
		*/
		$('.product-addon input, .product-addon textarea, .product-addon select, .product-addon input.qty, .wc-pao-addon input, .wc-pao-addon textarea, .wc-pao-addon select, .wc-pao-addon-image-swatch, .wc-pao-addon input.qty').on( 'woocommerce-product-addons-update', function() {
